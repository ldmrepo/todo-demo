...
import { Todo, Category, Priority, TodoTemplate, UserSettings, StatsData } from '../db';
...
interface TodoState {
  ...
  refreshData: () => void;
  addTodo: (todo: Omit<Todo, 'id'>) => Promise<number | undefined>;
  ...
  // 템플릿 액션
  addTemplate: (template: Omit<TodoTemplate, 'id'>) => Promise<TodoTemplate | undefined>;
  ...
  toggleSubtask: (todoId: number, subtaskId: string, completed: boolean) => Promise<void>;
...
const filterTodos = (
  ...
  return todos.filter(todo => {
    ...
    if (selectedTags.length > 0) {
      if (!todo.tags || !selectedTags.every(tag => todo.tags.includes(tag))) {
        return false;
      }
    }
    ...
    if (selectedCategories.length > 0) {
      if (!todo.categoryId || !selectedCategories.includes(todo.categoryId)) {
        return false;
      }
    }
    ...
    if (searchQuery.trim()) {
      ...
      const tagsMatch = todo.tags ? todo.tags.some(tag => tag.toLowerCase().includes(query)) : false;
      const notesMatch = todo.notes ? todo.notes.some(note => note.content.toLowerCase().includes(query)) : false;
...
export const useTodoStore = create<TodoState>((set, get) => ({
  ...
  loadData: async () => {
    ...
    try {
      ...
      set({ todos, categories, templates, settings, stats, isLoading: false });
    } catch (error) {
  ...
  setSelectedPriority: (priority: Priority | null) => set({ selectedPriority: priority }),
  setSearchQuery: (query: string) => set({ searchQuery: query }),
  setCompletionFilter: (filter: 'all' | 'completed' | 'active') => set({ completionFilter: filter }),
  clearFilters: () => set({
    selectedTags: [],
    selectedCategories: [],
    selectedPriority: null,
  ...
  setCalendarViewType: (type: 'day' | 'week' | 'month' | 'year') => set({ calendarViewType: type }),
  ...
  toggleSubtask: async (todoId: number, subtaskId: string, completed: boolean) => {
    try {
