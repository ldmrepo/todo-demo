      ...
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        try {
          ...
          if (oldVersion < 2 && oldVersion >= 1) {
            console.log('Upgrading to version 2 (adding tags field)');
            // 이미 생성된 스토어만 사용
        ...
        try {
          ...
          if (oldVersion < 3) {
            console.log('Upgrading to version 3 (adding new stores and fields)');
            ...
            if (db.objectStoreNames.contains(TODO_STORE)) {
              ...
              // 필요한 인덱스 추가
              const indexesToAdd = [
                'startDate', 'endDate', 'priority', 'categoryId', 
                'parentId', 'completedAt'
              ];
...
// 날짜 범위의 할 일 조회 함수
export const getTodosByDateRange = async (startDate: Date, endDate: Date): Promise<Todo[]> => {
  const db = await initDB();
  const allTodos = await db.getAll(TODO_STORE);
...
// 우선순위별 할 일 조회
export const getTodosByPriority = async (priority: Priority): Promise<Todo[]> => {
  const db = await initDB();
...
// 특정 카테고리의 할 일 조회
export const getTodosByCategory = async (categoryId: string): Promise<Todo[]> => {
  const db = await initDB();
...
export const getTodosByAnyTags = async (tags: string[]): Promise<Todo[]> => {
  ...
  return allTodos.filter(todo => 
    todo.tags && todo.tags.some(tag => tags.includes(tag))
  );
};
...
export const getTodosByAllTags = async (tags: string[]): Promise<Todo[]> => {
  ...
  return allTodos.filter(todo => 
    todo.tags && tags.every(tag => todo.tags.includes(tag))
  );
};

// 완료/미완료 할 일 조회
export const getTodosByCompletion = async (completed: boolean): Promise<Todo[]> => {
  const db = await initDB();
  const allTodos = await db.getAll(TODO_STORE);
...
export const searchTodos = async (query: string): Promise<Todo[]> => {
  ...
  return allTodos.filter(todo => 
    ...
    (todo.notes && todo.notes.some(note => note.content.toLowerCase().includes(lowerQuery)))
  );
};
...
// 할 일 업데이트
export const updateTodo = async (todo: Todo): Promise<number> => {
  const db = await initDB();
  return db.put(TODO_STORE, todo);
};

// 할 일 완료 상태 토글
export const toggleTodoCompletion = async (id: number, completed: boolean): Promise<Todo | undefined> => {
  const db = await initDB();
...
// 하위작업 추가
export const addSubtask = async (todoId: number, subtaskText: string): Promise<Todo | undefined> => {
  const db = await initDB();
...
// 하위작업 상태 토글
export const toggleSubtask = async (todoId: number, subtaskId: string, completed: boolean): Promise<Todo | undefined> => {
  const db = await initDB();
  const todo = await db.get(TODO_STORE, todoId);
...
// 메모 추가
export const addNote = async (todoId: number, content: string): Promise<Todo | undefined> => {
  const db = await initDB();
  const todo = await db.get(TODO_STORE, todoId);
...
// 시간 추적 시작
export const startTimeTracking = async (todoId: number): Promise<Todo | undefined> => {
  const db = await initDB();
  const todo = await db.get(TODO_STORE, todoId);
...
// 시간 추적 종료
export const stopTimeTracking = async (todoId: number): Promise<Todo | undefined> => {
  const db = await initDB();
  const todo = await db.get(TODO_STORE, todoId);
...
// 템플릿 관련 함수
export const addTemplate = async (template: Omit<TodoTemplate, 'id'>): Promise<TodoTemplate> => {
  const db = await initDB();
...
export const updateTemplate = async (template: TodoTemplate): Promise<TodoTemplate> => {
  const db = await initDB();
...
// 템플릿으로 할 일 생성
export const createTodoFromTemplate = async (templateId: string): Promise<number | undefined> => {
  const db = await initDB();
...
// 특정 기간의 통계 가져오기
export const getStatsRange = async (startDate: Date, endDate: Date): Promise<StatsData[]> => {
  const db = await initDB();
