...
export function TodoItem({ todo, onToggle, onDelete, onEdit }: TodoItemProps) {
    const { toggleSubtask, addNote, startTimeTracking, stopTimeTracking } =
        useTodoStore();
    ...
    const formatDateTime = (date: Date) => {
        ...
        const hasTime =
            todoDate.getHours() !== 0 || todoDate.getMinutes() !== 0;

        if (isSameDay) {
            return hasTime ? `오늘 ${format(todoDate, "HH:mm")}` : "오늘";
        }
            ...
            case "daily":
                return `매일 반복${
                    todo.repeat.interval && todo.repeat.interval > 1
                        ? ` (${todo.repeat.interval}일마다)`
                        : ""
                }`;
            case "weekly":
                return `매주 반복${
                    todo.repeat.interval && todo.repeat.interval > 1
                        ? ` (${todo.repeat.interval}주마다)`
                        : ""
                }`;
            case "monthly":
                return `매월 반복${
                    todo.repeat.interval && todo.repeat.interval > 1
                        ? ` (${todo.repeat.interval}개월마다)`
                        : ""
                }`;
            case "yearly":
                return `매년 반복${
                    todo.repeat.interval && todo.repeat.interval > 1
                        ? ` (${todo.repeat.interval}년마다)`
                        : ""
                }`;
            case "custom":
                return "사용자 정의 반복";
    ...
    return (
        <>
            <div
                className={`flex flex-col p-3 rounded-md border mb-2 ${
