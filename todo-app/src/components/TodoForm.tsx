...
import { Todo, Priority, RepeatType, RepeatConfig, Category } from '../db';
...
export function TodoForm({ onSubmit, onCancel, initialTodo }: TodoFormProps) {
  ...
  const [calendarMode, setCalendarMode] = useState<'dueDate' | 'startDate' | 'endDate' | 'repeatEndDate'>('dueDate');
  ...
  useEffect(() => {
    if (initialTodo) {
      ...
      if (initialTodo.subtasks) {
        setSubtasks(initialTodo.subtasks.map(st => ({
          id: st.id,
          text: st.text,
    ...
    try {
      ...
      if (repeatType !== RepeatType.NONE) {
        repeatConfig = {
          ...
          daysOfWeek: repeatType === RepeatType.WEEKLY ? repeatDaysOfWeek : undefined,
          endDate: repeatEndDate,
          occurrence: repeatOccurrence > 0 ? repeatOccurrence : undefined
        };
      }
  ...
  const formatDateTime = (date: Date | undefined, includeTime: boolean = true) => {
    if (!date) return '날짜 선택';
    
    let formattedDate = format(date, 'yyyy년 MM월 dd일');
    
    if (includeTime && selectedHour !== undefined && selectedMinute !== undefined) {
        ...
        <div className="flex mt-1 space-x-2">
          {days.map((day, index) => (
            <Button
              ...
              type="button"
              size="sm"
              variant={repeatDaysOfWeek.includes(index) ? 'default' : 'outline'}
              onClick={() => {
                if (repeatDaysOfWeek.includes(index)) {
                  setRepeatDaysOfWeek(repeatDaysOfWeek.filter(d => d !== index));
                } else {
                  setRepeatDaysOfWeek([...repeatDaysOfWeek, index]);
                }
              }}
            >
              {day}
  ...
  const openCalendarDatePicker = (mode: 'dueDate' | 'startDate' | 'endDate' | 'repeatEndDate') => {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="basic" className="space-y-4">
          ...
          {/* 태그 입력 필드 */}
          <div>
            <label className="text-sm font-medium block mb-1">태그</label>
            <TagInput
              tags={tags}
          ...
          <div>
            ...
            <Select value={categoryId || '_none'} onValueChange={(value) => setCategoryId(value === '_none' ? undefined : value)}>
              <SelectTrigger>
                <SelectValue placeholder="카테고리 선택" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="_none">카테고리 없음</SelectItem>
          ...
          <div>
            ...
            >
              {dueDate ? formatDateTime(dueDate) : '날짜 선택하기'}
            </Button>
          </div>
        </TabsContent>
        ...
        <TabsContent value="advanced" className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Button 
                type="button" 
                variant="outline" 
                onClick={() => openCalendarDatePicker('startDate')}
                className="w-full justify-start text-left"
              >
                {startDate ? formatDateTime(startDate, false) : '날짜 선택하기'}
              </Button>
            </div>
            <div>
              ...
              <Button 
                type="button" 
                variant="outline" 
                onClick={() => openCalendarDatePicker('endDate')}
                className="w-full justify-start text-left"
              >
                {endDate ? formatDateTime(endDate, false) : '날짜 선택하기'}
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <label className="text-sm font-medium block">반복 설정</label>
            <Select value={repeatType} onValueChange={(value) => setRepeatType(value as RepeatType)}>
              <SelectTrigger>
                <SelectValue placeholder="반복 유형 선택" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={RepeatType.NONE}>반복 안함</SelectItem>
                <SelectItem value={RepeatType.DAILY}>매일</SelectItem>
                <SelectItem value={RepeatType.WEEKLY}>매주</SelectItem>
                <SelectItem value={RepeatType.MONTHLY}>매월</SelectItem>
                <SelectItem value={RepeatType.YEARLY}>매년</SelectItem>
                <SelectItem value={RepeatType.CUSTOM}>사용자 정의</SelectItem>
              </SelectContent>
            </Select>
              ...
              <>
                {repeatType !== RepeatType.CUSTOM && (
                  <div className="grid grid-cols-2 gap-2 items-end">
                    <div>
                      <label className="text-sm font-medium block mb-1">간격</label>
                      <div className="flex items-center">
                        <Input
                          type="number"
                        ...
                        />
                        <span className="ml-2">
                          {repeatType === RepeatType.DAILY ? '일' : 
                           repeatType === RepeatType.WEEKLY ? '주' : 
                           repeatType === RepeatType.MONTHLY ? '월' : '년'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {repeatType === RepeatType.WEEKLY && renderDayOfWeekSelector()}
                
                <div className="space-y-2">
                  <label className="text-sm font-medium block">종료 설정</label>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm block mb-1">반복 횟수</label>
                      <Input
                        type="number"
                        value={repeatOccurrence || ''}
                        onChange={(e) => setRepeatOccurrence(parseInt(e.target.value) || 0)}
                        placeholder="무제한"
                        min={0}
                        className="w-full"
                      />
                    ...
                    <div>
                      <label className="text-sm block mb-1">종료일</label>
                      <Button 
                        type="button" 
                        variant="outline" 
                        onClick={() => openCalendarDatePicker('repeatEndDate')}
                        className="w-full justify-start text-left"
                      >
                        {repeatEndDate ? formatDateTime(repeatEndDate, false) : '날짜 선택하기'}
                      </Button>
                    </div>
                  </div>
                </div>
        ...
        <TabsContent value="subtasks" className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium block">하위 작업</label>
          ...
          {subtasks.length > 0 ? (
            <div className="space-y-2">
              {subtasks.map((task, index) => (
                <div key={task.id || index} className="flex items-center space-x-2 border p-2 rounded">
                  <Input
                    value={task.text}
                    onChange={(e) => {
            ...
            <p className="text-muted-foreground text-sm">하위 작업이 없습니다. 위에서 추가해보세요!</p>
          )}
        </TabsContent>
      </Tabs>
      ...
      {showCalendar && (
        <div className="mt-2 border rounded-md p-3 bg-background">
          <Calendar
            mode="single"
            selected={
              calendarMode === 'dueDate' ? dueDate : 
              calendarMode === 'startDate' ? startDate : 
              calendarMode === 'endDate' ? endDate : repeatEndDate
            }
      ...
      <Dialog open={showNewCategoryDialog} onOpenChange={setShowNewCategoryDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-3">
            <div>
              <label className="text-sm font-medium block mb-1">카테고리 이름</label>
              <Input
            ...
            <div>
              ...
              <div className="grid grid-cols-6 gap-2">
                {['#EF4444', '#F97316', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#EC4899'].map(color => (
                  <div
                    key={color}
                    className={`w-8 h-8 rounded-full cursor-pointer border-2 ${newCategoryColor === color ? 'border-white ring-2 ring-black' : 'border-transparent'}`}
