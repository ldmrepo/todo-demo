...
export function StatisticsView() {
    ...
    // 완료된 작업 vs 전체 작업
    const completedTotal = periodStats.reduce(
        (sum, stat) => sum + stat.completedCount,
        0
    );
    const tasksTotal = periodStats.reduce(
        (sum, stat) => sum + stat.totalCount,
        0
    );
    const completionRate =
        tasksTotal > 0 ? (completedTotal / tasksTotal) * 100 : 0;
    ...
    // 우선순위별 할 일 통계
    const priorityStats = {
        [Priority.HIGH]: todos.filter((todo) => todo.priority === Priority.HIGH)
            .length,
        [Priority.MEDIUM]: todos.filter(
            (todo) => todo.priority === Priority.MEDIUM
        ).length,
        [Priority.LOW]: todos.filter((todo) => todo.priority === Priority.LOW)
            .length,
    };
    ...
    const priorityChartData = [
        {
            label: "높음",
            value: priorityStats[Priority.HIGH],
            color: "#EF4444",
        },
        {
            label: "중간",
            value: priorityStats[Priority.MEDIUM],
            color: "#F59E0B",
        },
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm">
                                {priorityStats[Priority.HIGH]} 높음
                            </span>
                        </div>
                        <div className="flex items-center space-x-1">
                            <div className="w-3 h-3 rounded-full bg-amber-500"></div>
                            <span className="text-sm">
                                {priorityStats[Priority.MEDIUM]} 중간
                            </span>
                        </div>
                        <div className="flex items-center space-x-1">
                            <div className="w-3 h-3 rounded-full bg-green-500"></div>
                            <span className="text-sm">
                                {priorityStats[Priority.LOW]} 낮음
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-background border rounded-lg p-4 shadow-sm">
                ...
                <div className="space-y-4">
                    {activeTodos
                        .filter(
                            (todo) =>
                                todo.timeTracking?.started &&
                                !todo.timeTracking?.stopped
                        )
                        .map((todo, index) => {
                            // 현재 진행 시간 계산
                            let totalSeconds = 0;
                            if (todo.timeTracking?.totalTime) {
                                totalSeconds = Math.floor(
                                    todo.timeTracking.totalTime / 1000
                                );
                            }
