...
export function CategoryView() {
  ...
  // 계층 구조 카테고리 목록 만들기
  const buildCategoryTree = (items: Category[], parentId?: string): Category[] => {
    return items
      ...
      <Dialog open={showNewDialog} onOpenChange={setShowNewDialog}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">상위 카테고리 (선택사항)</label>
              <select 
            ...
            <div className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-8 gap-2">
                  {colorPalette.map((color) => (
                    <div
                      key={color}
                      className={`w-8 h-8 rounded-full cursor-pointer border-2 ${
                        editCategory.color === color ? 'border-black' : 'border-transparent'
                      }`}
                      style={{ backgroundColor: color }}
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="">상위 카테고리 없음</option>
                  {categories
                    .filter(category => category.id !== editCategory.id) // 자기 자신은 제외
                    .map(category => (
                      <option key={category.id} value={category.id}>
                        {category.name}
                      </option>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="py-4">
              ...
              <p className="text-muted-foreground text-sm mt-2">
                * 이 카테고리에 속한 할 일들은 카테고리 없는 상태로 변경됩니다.
              </p>
            </div>
            <DialogFooter>
